...
function titleCase(title, minorWords) {
    ...
    let res = ''
    if (minorWords) {
        minorWords = minorWords.split(" ").map(el => el.toLowerCase())
    }
    const lowerTitle = title.split(" ").map(el => el.toLowerCase())

    for (let i = 0; i < lowerTitle.length; i++) {
        if (minorWords && minorWords.includes(lowerTitle[i])) continue

        if (lowerTitle[i] && lowerTitle[i].length > 0) {
            lowerTitle[i] = lowerTitle[i][0].toUpperCase() + lowerTitle[i].slice(1);
        }
    }
    lowerTitle[0] = lowerTitle[0][0].toUpperCase() + lowerTitle[0].slice(1)
